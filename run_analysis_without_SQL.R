...
# eventually got using sapply and split

library(tidyr)
rest<-sapply( split(mean_std_lab[,3:88], list(mean_std_lab$subject, mean_std_lab$activity) ) , colMeans)
#change matrix to data frame
restd<- as.data.frame(rest)
#put rownames as first column, delete rownames and rename the first column- 3 steps
restd_bind<- cbind( row.names(restd), restd)
rownames(restd_bind)<-NULL
colnames(restd_bind)[grep( "row\\.names\\(restd\\)" ,names(restd_bind) ) ]<- "average_of_variable"
#first gather and then separate
rest_final<-gather(restd_bind, subject.activity, normalised_average_value, -c("average_of_variable"))
rest_final<-separate(rest_final, "subject.activity", c("subject", "activity"), sep="\\.")

# Drop the unused objects/tables/dataframes and variables
objects<- as.list( ls())
objects[ c( grep("rest_final", objects) , grep("mean_std_lab", objects) ) ]<-NULL
rm( list =  as.character(objects) )
rm(objects)
